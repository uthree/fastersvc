...
### AdaINによるスタイル変換(実験的な機能)
`-adain True` オプションをつけることによりAdaINによるスタイル変換機能を有効にすることができます。リアルタイム推論では使用できません。

## デコーダーの学習について
デコーダーを学習する際に`--weight-mel <実数>`で対数メルスペクトログラム損失の重みを設定することができます。(デフォルトは`5.0`)  
この数値を調整することにより、話者性の再現度が変化することがあります。
